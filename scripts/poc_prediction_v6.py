"""
PoC v6: 68 Features — Technical Indicators + Fourier + Distribution + Advanced Supply.

27 new features over v5's 41:
  - Technical Indicators (8): EMA, MACD, Bollinger Bands, RSI, momentum
  - Fourier/Cyclical (6): sin/cos at annual, semi-annual, weekly periods
  - Advanced Calendar (5): pre-holiday, pre-weekend, consecutive gap, week position
  - Price Distribution (4): skewness, kurtosis, percentile, z-score
  - Advanced Supply (4): YoY ratio, origin diversity, lot size, high-low spread

Usage:
    uv run python scripts/poc_prediction_v6.py
"""
import json
import warnings
...
# ── Feature Engineering (v6: 68 features) ───────────────────────────
...
def build_features_v6(records, ctx, target_sp, offset=7, use_smoothed=False):
    ...
    # Week-of-year averages for YoY comparison
    ...
    for i in range(90, len(records) - offset):  # Need 90 for percentile_90d
        ...
        # --- Advanced Supply ---
        # YoY: compare current week qty to same week last year
        ...
        # Origin diversity in last 7 days
...
def backtest(X, y, fnames, prices_raw, species, horizon, method="vmd", n_splits=5):
    ...
    result = {"species": species, "model": f"v6-{method}", "horizon": horizon,
...
def main():
    ...
    for cfg in SPECIES_CONFIGS:
        ...
        print(f"{'='*70}")
        print(f"  {cfg['label']} — {method}, 68 features")
        print(f"{'='*70}")
        ...
        if cfg.get("regime_split"):
            ...
            for horizon in [7]:
                X, y, fnames, dates = build_features_v6(records, ctx, sp, horizon, cfg.get("smoothed", False))
    ...
    # Feature importance analysis: which NEW features matter?
    print("\n" + "=" * 80)
    print("NEW FEATURE IMPACT ANALYSIS")
    print("=" * 80)
    new_features = {
    ...
    for cat in new_features:
        print(f" {cat:>12}", end="")
    print(f" {'Total New':>10}")
    print(f"  {'-'*80}")

    for sp, imp in all_importance.items():
        print(f"  {sp:<15}", end="")
        total_new = 0
        for cat, feats in new_features.items():
            cat_total = sum(imp.get(f, 0) for f in feats)
            total_new += cat_total
            print(f" {cat_total:>11.1f}%", end="")
        print(f" {total_new:>9.1f}%")

    # Comparison
    print("\n" + "=" * 80)
    print("v5 vs v6 COMPARISON (7-day horizon)")
    print("=" * 80)
    v5p = OUTPUT_DIR / "poc_v5_results.json"
    v5_data = {}
    if v5p.exists():
        with open(v5p) as f:
            for sp, info in json.load(f).get("summary", {}).items():
                v5_data[sp] = info.get("v5")

    print(f"\n  {'Species':<25} {'v5':>8} {'v6':>8} {'Δ':>8} {'v6 Dir%':>7}")
    print(f"  {'-'*58}")
    summary = {}
    for cfg in SPECIES_CONFIGS:
        sp = cfg["species"]
        if cfg.get("regime_split"):
            v6r = next((r for r in all_results if r["species"] == f"{sp}_winter"), None)
            label = f"{sp} (winter)"
        else:
            v6r = next((r for r in all_results if r["species"] == sp), None)
            label = sp
        if not v6r: continue
        v5 = v5_data.get(sp, v6r["mape"])
        delta = f"{(v5 - v6r['mape']) / v5 * 100:+.0f}%" if v5 else "—"
        print(f"  {label:<25} {v5:>7.1f}% {v6r['mape']:>7.1f}% {delta:>8} {v6r['dir_acc']:>6.1f}%")
        summary[sp] = {"v5": v5, "v6": v6r["mape"], "dir_acc": v6r["dir_acc"],
    ...
    out = {
        ...
        "total_features": 68,
        "new_features_added": 27,
        "results": all_results,
        "feature_importance": {k: dict(list(v.items())[:20]) for k, v in all_importance.items()},
        "summary": summary,
    }
    with open(OUTPUT_DIR / "poc_v6_results.json", "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    print(f"\nSaved to {OUTPUT_DIR / 'poc_v6_results.json'}")
